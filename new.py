...
if not st.session_state.authenticated:
    ...
    code_input = st.text_input("Enter 6-digit passcode", type="password", key="login_pass")
    if st.button("Login"):
...
def connect_sheets():
    ...
    return {
        ...
        "income": sh.worksheet("Other Income"),
...
# TABS
# ---------------------------
tabs = st.tabs(["📊 Dashboard","👥 Customers","📝 Orders","💳 Transactions","🧾 Expenses","💰 Other Income","📦 Inventory","📄 Invoices","⚙️ Settings"])
...
with tabs[1]:
    ...
    if st.session_state.user_role=="admin":
        with st.expander("➕ Add Customer"):
            ...
            if st.button("Save Customer", key="save_customer_new"):
...
with tabs[2]:
    ...
    if st.session_state.user_role=="admin" and not customers.empty:
        with st.expander("➕ Add Order"):
            ...
            if st.button("Save Order", key="save_order_new"):
...
with tabs[3]:
    ...
    if st.session_state.user_role=="admin" and not orders.empty:
        with st.expander("➕ Add Transaction"):
            ...
            if st.button("Save Transaction", key="save_trans_new"):
...
with tabs[4]:
    ...
    if st.session_state.user_role=="admin":
        with st.expander("➕ Add Expense"):
            ...
            if st.button("Save Expense", key="save_expense_new"):
                eid = len(expenses)+1
...
with tabs[5]:
    ...
    if st.session_state.user_role=="admin":
        with st.expander("➕ Add Income"):
            ...
            if st.button("Save Income", key="save_income_new"):
                iid = len(income)+1
...
with tabs[6]:
    ...
    if st.session_state.user_role=="admin":
        with st.expander("➕ Add Item"):
            ...
            qty = st.number_input("Quantity", min_value=0, key="inventory_qty")
            unit_price = st.number_input("Unit Price", min_value=0.0, key="inventory_unit_price")
            if st.button("Save Item", key="save_inventory_new"):
                iid = len(inventory)+1
...
with tabs[7]:
    ...
    oid_select = st.selectbox("Select Order ID for Invoice", orders["Order ID"] if not orders.empty else [], key="invoice_order")
    if oid_select and st.button("Generate Invoice", key="generate_invoice"):
...
with tabs[8]:
    ...
    feedback = st.text_area("Share feedback or report issues", key="settings_feedback")
